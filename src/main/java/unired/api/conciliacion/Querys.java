...
public class Querys {
    ...
    public static final String CONCILIACION_FECHA_ACTUAL = ""
            + "SELECT\n"
    ...
    public static final String HISTORIC_CONCILIACION = ""
            + "SELECT \n"
            + "	A.IDEMPRESA,\n"
            ...
            + "		WHERE \n"
            + "			(A.IdEmpresa  = 'GEOCOM') \n"
            + "			AND (CONVERT(DATE, A.FechaCreacion) BETWEEN '${fechaIni}' AND '${fechaFin}') -- FechaIini y FechaFin como parámetros\n"
            + "			AND (DATEPART(HOUR, A.FechaCreacion) >= 1 AND DATEPART(MINUTE, A.FechaCreacion) >=20) \n"
            + "			AND (DATEPART(HOUR, A.FechaCreacion) <= 1 AND DATEPART(MINUTE, A.FechaCreacion) <=25 AND DATEPART(SECOND, A.FechaCreacion) <= 59)\n"
            + "		GROUP BY \n"
